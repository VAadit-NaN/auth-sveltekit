<script lang="ts">
	...
	export let data;
	const signInSchema = userSchema.pick({ username:true, email: true, password: true , confirmPassword: true});
...
<Register href="/">
    ...
    <form class="flex flex-col space-y-6" method="POST" action="?/signup" use:enhance>
        ...
        <span>Your username</span>
            <input id = "username" type="username" name="username" placeholder="R_Hendricks" data-invalid={$errors.username} bind:value={$form.username} class="input" required />
        </label>
        ...
        <input id="confirmPassword" type="password" name="confirmPassword" placeholder="••••" data-invalid={$errors.password} bind:value={$form.confirmPassword} class="input" class:input-error={$errors.password} required />
        </label>
        ...
        Already Registered? <a href="/login" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Log in</a>
        </p>
    </form>
    </div>
</Register>
